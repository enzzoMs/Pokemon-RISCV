...
# na matriz de movimentação, caso o valor dessa posição seja maior ou igual a 32 (1_000_00 em binário)   #
...
RENDERIZAR_AREA:
	...
	# está prester a sair de uma área, além disso é necessário perguntar se o jogador quer mesmo sair
		# A única exceção para esse caso é se a0 = 1_000_00, em que a próxima área é o quarto do RED
		# entrando por lugar nenhum, ou seja, o jogo está começando
		
		li t0, 32	# 32 = 1_000_00 em binário
...
TRANSICAO_ENTRE_AREAS:
	...
	TRANSICAO_SETA_CIMA:
		...
		bne s1, t1, TRANSICAO_SETA_BAIXO
			...
			sub t3, s5, s3	# o endereço de onde a seta vai estar é acima da posição do RED
	...
	# Agora é necessário imprimir a mensagem indicando qual tecla apertar para sair da área
		# Essa mensagem sempre fica em uma posição fixa no frame 0 que é calculada abaixo
			li a1, 0xFF000000		# seleciona como argumento o frame 0
			li a2, 256 			# numero da coluna 
		...
		la a0, mensagem_sair_area	# carrega a imagem da mensagem
...
NAO_SAIR_DA_AREA:
	...
	mv t4, a0	# salva a0 (tecla apertada) em t4
	...
	# Limpando mensagem de transição de área
		# Para isso é necessário limpar 8 tiles em 2 linhas, eles sempre são os mesmos independente 
		# da área. O endereço do primeiro deles está na 13a linha e 16a coluna de tiles a partir de s2
		...
		addi t0, t0, 16		# move t0 por mais 16 colunas
		add t3, s2, t0		# t3 tem o endereço do 1o tile a ser limpo
	
		li t5, 0		# contador para o numero de tiles a serem limpos
		li t6, 4		# numero de tiles a serem limpos
		
		# Limpando a primeira linha de tiles
		LOOP_LIMPAR_MENSAGEM_DE_TRANSICAO_1_LINHA:
			add a4, t3, t5		# limpa o tile de numero t5 a partir de t3
			li a5, 0xFF000000	# a5 recebe o endereço base do frame 0		
			call LIMPAR_TILE

			addi t5, t5, 1		# incrementa o numero de loops realizados
			bne t5, t6, LOOP_LIMPAR_MENSAGEM_DE_TRANSICAO_1_LINHA	# reinicia o loop se t5 != t6
	
		li t5, 0		# contador para o numero de tiles a serem limpos
		li t6, 4		# numero de tiles a serem limpos
		
		add t3, t3, s3		# passa o endereço de t3 para a proxima linha na matriz de tiles
		
		LOOP_LIMPAR_MENSAGEM_DE_TRANSICAO_2_LINHA:
			add a4, t3, t5		# limpa o tile de numero t5 a partir de t3
			li a5, 0xFF000000	# a5 recebe o endereço base do frame 0		
			call LIMPAR_TILE

			addi t5, t5, 1		# incrementa o numero de loops realizados
			bne t5, t6, LOOP_LIMPAR_MENSAGEM_DE_TRANSICAO_2_LINHA	# reinicia o loop se t5 != t6							
	...
	mv a0, t4	# t4 ainda tem a tecla que foi apertada pelo jogador, elá sera usada em a0 para
