...
RENDERIZAR_ESCOLHA_DE_POKEMON_INICIAL:
	...
	# atualizar a matriz de movimentação para que não seja possível escolher um pokemon novamente,
	...
	# tiles de pallet
		...
		# Agora será liberado a saída de pallet
		# Para isso os dois primeiros tiles de grama de pallet, aqueles que inicialmente chamaram 
		# RENDERIZAR_ANIMACAO_PROF_OAK, serão atualizados para o codigo 7 (codigo de tile de grama)
		
		la t0, matriz_movimentacao_pallet
		lw t1, 0(t0)		# t1 = largura da matriz
		lw t2, 4(t0)		# t2 = altura da matriz
		addi t0, t0, 8		# pula para onde começa os elementos na matriz
				
		addi t2, t2, -21	# o endereço de onde estão os tiles de grama é a -21 linhas
		mul t1, t1, t2		# e + 12/13 colunas da linha final da matriz
		add t0, t0, t1	
		
		li t1, 7		# atualiza os endereços com o codigo do tile de grama (7)
		sb t1, 12(t0)
		sb t1, 13(t0)	

		# Atualizando a matriz de tiles de pallet
		# Como pallet é muito grande, para impedir que o RED fique muito embaixo da tela na hora
		# de renderizar os dialogos uma linha da matriz é substituida temporariamente por -1s, impedindo
		# que o restante do mapa seja renderizado, agora é necessário substituir esses -1s pelos valores
		# corretos dos tiles para que o RED possa sair
		...
		addi t2, t2, -29	# o endereço de onde esta a linha a ser atualizada é a -29 linhas
		mul t1, t1, t2		# + 1 coluna do final da matriz de tiles
		add t0, t0, t1	
		addi t0, t0, 1
		
		li t1, 24		# numero de posicoes da matriz a serem atualizadas
		la t2, NOVA_LINHA_DE_TILES_PALLET	
		LOOP_ATUALIZAR_TILES_PALLET:
			lb t3, 0(t2)		# pega o elemento de NOVA_LINHA_DE_TILES_PALLET
			sb t3, 0(t0)		# armazena o elemento na matriz de tiles
			
			addi t0, t0, 1		# incrementa o endereço da matriz de tiles
			addi t2, t2, 1		# incrementa o endereço de NOVA_LINHA_DE_TILES_PALLET 
			addi t1, t1, -1		# decrementa o numero de loops restantes
		bne t1, zero, LOOP_ATUALIZAR_TILES_PALLET 	# reinicia o loop se t1 != 0
